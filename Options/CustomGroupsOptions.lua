...
local function EditAura(def, title, onChange, group)
    ...
    if AL.SourceProbed() and not AL.CanShowSource() then
        ...
        note:SetText("|cff888888Caster names arrive with Blizzard patch 12.1.5|r")
        f:AddChild(note)
    end
...
function TUI:CustomGroupsOptions()
    ...
    local function entriesFor(group, scope, key)
        ...
        if scope == "global" and ns.CustomGroups and ns.CustomGroups.TrinketDestFor then
            for _, slot in ipairs({ 13, 14 }) do
                local d = ns.CustomGroups.TrinketDestFor(slot)
                if d and d.group == group.id then
                    local itemID = GetInventoryItemID and GetInventoryItemID("player", slot)
                    out[#out + 1] = { kind = "trinket", id = slot, li = d.order or 15000, uid = "trinket:" .. slot,
                        name = (itemID and C_Item.GetItemInfo(itemID)) or ("Trinket " .. (slot == 13 and "1" or "2")),
                        tex  = (itemID and C_Item.GetItemIconByID and C_Item.GetItemIconByID(itemID)) or 134400 }
                end
            end
        end
                    ...
                    for i = 1, 50 do
                        ...
                        box["r" .. i .. "_link"] = {
                            ...
                            name = function()
                                ...
                                if e.kind == "trinket" then return "|cFFFFD200Trinkets|r" end
                            ...
                            hidden = function()
                                local e = entry()
                                return not (e and (e.kind == "timer" or e.kind == "specialicon"
                                    or e.kind == "aura" or e.kind == "trinket"))
                            end,
                            func = function()
                                local e = entry(); if not e then return end
                                if e.kind == "trinket" then
                                    if E.ToggleOptions then E:ToggleOptions("thingsUI,modulesTab,cdm,trinketBlacklistSubTab") end
                                    return
                                end
                                if e.kind == "aura" then
                        ...
                        box["r" .. i .. "_remove"] = {
                            ...
                            func = function()
                                ...
                                elseif e.kind == "trinket" then
                                    ...
                                    if db and db.trinketDest then db.trinketDest[tostring(e.id)] = nil end
    ...
    local function GroupEditor(group, index)
        ...
        return {
            ...
            name = function()
                local nm = group.name or "Group"
                local CGm = ns.CustomGroups
                if CGm and CGm.TrinketDestFor then
                    local a, b = CGm.TrinketDestFor(13), CGm.TrinketDestFor(14)
                    if (a and a.group == group.id) or (b and b.group == group.id) then
                        nm = nm .. " |cFFFFD200[T]|r"
                    end
                end
                return group.enabled and nm or ("|cFF888888" .. nm .. "|r")
            end,
            args = {
                enable = {
                ...
                specTab   = { order = 10, type = "group", args = specArgs,
                    name = function() return "Spec (" .. #entriesFor(group, "spec", getEditSpec()) .. ")" end },
                classTab  = { order = 11, type = "group", args = classArgs,
                    name = function() return "Class (" .. #entriesFor(group, "class", getEditClass()) .. ")" end },
                globalTab = { order = 12, type = "group", args = globalArgs,
                    name = function() return "Global (" .. #entriesFor(group, "global", nil) .. ")" end },
                itemsTab = {
                    ...
                    args = {
                        ...
                        qualityGroup = {
                            ...
                            args = {
                                ...
                                qX = { order = 5, type = "range", name = "X Offset", min = -32, max = 32, step = 0.01, bigStep = 1,
                                    disabled = function() return not group.qualityBorder end,
                                    get = function() return group.qualityXOffset or 0 end, set = function(_, v) gset("qualityXOffset", v) end },
                                qY = { order = 6, type = "range", name = "Y Offset", min = -32, max = 32, step = 0.01, bigStep = 1,
                                    disabled = function() return not group.qualityBorder end,
                ...
                layoutTab = {
                    ...
                    args = {
                        ...
                        growth = { order = 3, type = "select", name = "Growth Direction", values = ns.GROWTH.VALUES, sorting = ns.GROWTH.ORDER,
                        ...
                        wrapDir = {
                            ...
                            values = function()
                                ...
                                if g == "UP" or g == "DOWN" or g == "CENTERED_V" then return { RIGHT = "Right", LEFT = "Left" } end
                                return { DOWN = "Down", UP = "Up" }
                            end,
                            get = function()
                                local g = group.growth
                                local default = (g == "UP" or g == "DOWN" or g == "CENTERED_V") and "RIGHT" or "DOWN"
                        ...
                        positionGroup = {
                            ...
                            args = {
                                ...
                                anchorXOffset = { order = 4, type = "range", name = "X Offset", min = -400, max = 400, step = 0.01, bigStep = 1,
                                    hidden = isUIParent, get = function() return group.anchorXOffset end, set = function(_, v) gset("anchorXOffset", v) end },
                                anchorYOffset = { order = 5, type = "range", name = "Y Offset", min = -400, max = 400, step = 0.01, bigStep = 1,
                    ...
                    args = {
                        cdGroup = {
                            ...
                            args = {
                                ...
                                cdX = { order = 7, type = "range", name = "Text X", min = -50, max = 50, step = 0.01, bigStep = 1, disabled = noCD,
                                    get = function() return tdb().cooldownXOffset end, set = function(_, v) tset("cooldownXOffset", v) end },
                                cdY = { order = 8, type = "range", name = "Text Y", min = -50, max = 50, step = 0.01, bigStep = 1, disabled = noCD,
                        ...
                        countGroup = {
                            ...
                            args = {
                                ...
                                cntX = { order = 7, type = "range", name = "Text X", min = -50, max = 50, step = 0.01, bigStep = 1, disabled = noCount,
                                    get = function() return tdb().countXOffset end, set = function(_, v) tset("countXOffset", v) end },
                                cntY = { order = 8, type = "range", name = "Text Y", min = -50, max = 50, step = 0.01, bigStep = 1, disabled = noCount,
                        ...
                        stacksGroup = {
                            ...
                            args = {
                                stacksDesc = { order = 0, type = "description",
                                    name = "Buff |cFFFFD200stack count|r (e.g. Moonfire stacks) on the group's aura icons.\n" },
                                ...
                                stkX = { order = 7, type = "range", name = "Text X", min = -50, max = 50, step = 0.01, bigStep = 1, disabled = noStacks,
                                    get = function() return tdb().stacksXOffset end, set = function(_, v) tset("stacksXOffset", v) end },
                                stkY = { order = 8, type = "range", name = "Text Y", min = -50, max = 50, step = 0.01, bigStep = 1, disabled = noStacks,
    ...
    return {
        order = 8, type = "group", name = "Groups - Icons", childGroups = "tab",
        args = args,
    }
end
